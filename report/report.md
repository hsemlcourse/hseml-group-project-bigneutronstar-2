...
| Accuracy | 0.5024 |
| F1 macro | 0.3479 |
| **Hit Rate (thresh=0.59)** | **73.8% (110 Wins / 39 Losses)** |
| **Сделок** | **149** |
| Avg Return / сделку | +0.905% |
| Profit Factor | 4.25 |
| Sharpe Ratio (годовой) | 4.20 |

---

### 🛡️ Симуляция доходности и кривой капитала (Депозит $10,000, Комиссия 0.05%)

Для оценки применимости модели в бизнесе мы провели детальную симуляцию на тестовом Hold-out периоде с учетом транзакционных спредов и комиссий в размере **0.05%** на сделку:

1. **Консервативная стратегия (без плеча / 1x Leverage):**
   * **Чистая прибыль**: **+$24,922.77 (+249.2%)**
   * **Финальный баланс**: **$34,922.77**
   * **Максимальная просадка (Drawdown)**: **25.5%**
2. **Агрессивная стратегия (с плечом / 3x Leverage):**
   * **Чистая прибыль**: **+$419,667.46 (+4196.7%)**
   * **Финальный баланс**: **$429,667.46**
   * **Максимальная просадка (Drawdown)**: **58.4%**

#### 📈 Кривая доходности портфеля (Equity Curve):
![Кривая доходности капитала](equity_curve.png)

*Примечание: график сохранен в директории отчета: `report/equity_curve.png`.*

---

### 🎥 Демонстрация интерактивного UI дашборда
Веб-интерфейс демонстрирует реальную работу модели в веб-интерфейсе: отображение исторических данных, вероятности для каждого часа и сигналы BUY / SELL / HOLD с учётом калибровки уверенности в 59%:
...
**Архитектура:**
- **Backend:** FastAPI ([app/main.py](../app/main.py)), грузит `models/cp3_stacking_model.pkl` при старте. Эндпоинт `/api/latest` отдаёт последние ~720 часов цены, по каждой точке — вероятности всех трёх классов и итоговый сигнал с учётом порога 0.59.
- **Frontend:** React + Vite + Chart.js ([frontend/](../frontend/)). По клику на точку графика справа обновляется блок с Trade Signal, Model Confidence и вероятностями Down/Flat/Up.
...
![Главный экран дашборда: график цены и предсказание модели на последний момент](screenshot_dashboard.png)
...
![AI Prediction Analysis: разбор одной точки во времени](screenshot_detail.png)
